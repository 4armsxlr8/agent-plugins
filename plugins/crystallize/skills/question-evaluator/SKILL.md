...
本監査は質問の**論理** (前提・二択・誘導・喪失) を見る。`spec` が毎ラウンド必ず行う**試問** (文脈ゼロの読者にシートを渡し、選択肢の弁別・音読・内輪語を検査する) とは対象が違い、置き換えにはならない。ただし読みやすさの検査のうち文面だけで判定できる 2 点 (音読して意味が取れるか・説明なしの内輪語) は `legibility` として本監査も採点する — 監査を通った質問がユーザーに読めない、という抜けを防ぐため。
...
- **読めるか (`legibility`)**: 質問文・選択肢を音読して不自然な文、意味の取れない文を逐語で挙げる。タスク一行要約の語彙の外で説明なしに使われている内輪語・作業語・初見の造語を列挙する (3 語以上なら減点)。書き手は題材の全文脈を頭に持っているので、読めるかの自己判定は構造的に甘い
...
```json
{
  ...
  "feedback_structured": {
    "high":   [{"area": "<premise|dilemma|neutrality|options|loss|legibility>", "message": "<指摘 + 根拠 (裏取りしたファイル:行)>"}],
    "medium": [{"area": "...", "message": "..."}],
    "low":    [{"area": "...", "message": "..."}]
  },
...
| `legibility` | 音読して意味の取れない文が無いか。説明なしの内輪語・造語が 3 語以上無いか。指摘は逐語で挙げる (「分かりにくい」だけの指摘は不可) | `spec` §試問 の検査 3・4 (文面で判定できる部分) |

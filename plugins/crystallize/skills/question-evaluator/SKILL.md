...
- 各選択肢に、それを選んだ場合に**できなくなること・失われる体験**が否定形で明記されているか。肯定形の説明のみ (例: 「画像がすべて」) は減点 — 喪失が実感されないまま確定し、実装後の挙動確認で認識ズレとして発覚した実例がある
...
```json
{
  ...
  "quality": {
    ...
    "breakdown": {
      ...
      "loss_disclosure": <0-100>
    }
  },
  "feedback": "<high → medium → low を畳み込んだ string サマリ>",
  "feedback_structured": {
    "high":   [{"area": "<premise|dilemma|neutrality|options|loss>", "message": "<指摘 + 根拠 (裏取りしたファイル:行)>"}],
    "medium": [{"area": "...", "message": "..."}],
    "low":    [{"area": "...", "message": "..."}]
  },
...
基準の正本は `find-unknowns` SKILL.md「質問の関所」の 4 規則。同節が改訂されたら本表と `eval-schema.json` を追随させる。
...
| `loss_disclosure` | 各選択肢に、選んだ場合の喪失 (できなくなること・失われる体験) が否定形で明記されているか。肯定形の説明のみは減点。選択肢なしの質問は N/A (満点扱い) | 質問の関所 規則 4 (喪失の開示) |

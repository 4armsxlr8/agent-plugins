...
- **`skills/plan-create`** — derives a throwaway implementation plan from the spec, ordered by how likely each item is to change; a large spec is split into several plans (one commit each) only when the parts touch disjoint modules and can be verified on their own.

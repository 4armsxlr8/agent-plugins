...
1. ゴール 1 文が先頭行にある
2. 確定事項 (テスト・lint の検証コマンド 1 行を含む。挙動に関わる確定事項には**否定形の帰結** — その結果、何が起きない・見えない・できないか — が併記されている)
...
| `structure_completeness` | 構成 6 要素・検証コマンド 1 行・否定形の帰結の併記が揃っているか。帰結の検査は寛容に — 併記が皆無なら減点、個々の確定事項の書き漏れは low 指摘に留める (新設規約の厳密適用は偽陽性を生む) | Step 5 構成 1〜6 |

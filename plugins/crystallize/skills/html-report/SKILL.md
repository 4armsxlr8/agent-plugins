...
書き始める前に次を Read する。**読まずに書き始めない** — 部品の見た目と図の座標は文章では伝わらず、思い出しで書くと必ず前回と違うものになる。
...
マークアップの骨格・タブ・diff テーブル・ファイルナビJSは `<skill-dir>/references/templates.md`。**`<skill-dir>` = スキル本文の先頭に注入される「Base directory for this skill」の絶対パス**。同梱ファイルはこれを起点にした絶対パスで開く — fork の cwd は呼び出し元と一致する保証がなく、相対パスでは見つからない。`${CLAUDE_PLUGIN_ROOT}` は hooks や commands の設定では使えるが、fork したスキルの Bash では空だった (実測) ので使わない。
...
| 2 | 用語 | `<dl class="terms">` にタスク固有の語だけ。無ければ節ごと省く(一般的な技術用語は書かない) |
...
- `assets/component-samples.html` — **全部品を1枚に並べた見本(見た目の正本)**。生成前に必ず Read する。単体でブラウザで開ける

...
3. 守るべき既存挙動
4. 実装計画が変わりやすい順に並んでいる (ユーザーが差し替えたくなるものが先頭、機械的作業が末尾)。**各項目に対応する受け入れ基準 ID (AC-n) と TDD 対象 / 対象外が添えてある**。spec に「見た目の正本」節があり「該当なし」でないなら、UI を含む項目 (実機確認に委ねる基準・モック確定印の要件に対応する項目) に `mock.html#variant=<id>` の参照が添えてあるか — **参照の有無だけを見る。mock.html 自体は開かない** (モックを読まない規則はそのまま)
...
| `structure_completeness` | 構成 5 要素 (ゴール 1 文 / `spec:` 行 / 守るべき既存挙動 / 実装計画 / 検証コマンド + Deviations 規約) と、各項目の受け入れ基準対応・TDD 適否の併記、UI 項目の見た目の正本への参照 (spec に同節があり「該当なし」でないとき) が揃っているか。併記の検査は寛容に — 皆無なら減点、個々の項目の書き漏れは low 指摘に留める | plan 構成 1〜5 |
